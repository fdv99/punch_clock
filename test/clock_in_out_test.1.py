...
users = {"harrisong": "greer", "chasev": "mcv", "blaked": "batman", "forrest": "fdv"}
...
def clockInSheet(
    user, date1, log_time, week_day, month, year, next_month, last_previous_month
):
    ...
    if os.path.isfile(f"{user}_{year_str}.{month_str}.xlsx") == True:
        ...
        lastMonthYear = int(
            (datetime.datetime.now().replace(day=1) - datetime.timedelta(days=1)).year
        )
        lastMonth = int(
            (datetime.datetime.now().replace(day=1) - datetime.timedelta(days=1)).month
        )
        currentMonthYear = int(datetime.datetime.today().year)
        currentMonth = int(datetime.datetime.today().month)
        nextMonthYear = (
            datetime.datetime.now().replace(day=28) + datetime.timedelta(days=4)
        ).year
        nextMonth = (
            datetime.datetime.now().replace(day=28) + datetime.timedelta(days=4)
        ).month
